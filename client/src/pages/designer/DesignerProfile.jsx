...
import { DesignerProfileBody, ErrorState, Spinner } from "../../components";
import {
...
const DesignerProfile = () => {
  ...
  useEffect(() => {
    dispatch(getMyProfileDesigner()).then((actionResult) => {
      if (getMyProfileDesigner.rejected.match(actionResult)) {
  ...
  const handleCreateProfile = async () => {
    ...
    const actionResult = await dispatch(createProfileDesigner(formData));
    if (createProfileDesigner.fulfilled.match(actionResult)) {
      setIsEditing(false);
      toast.name(
        "Your designer profile created seucessfully now others can see you"
      );
    } else if (createProfileDesigner.rejected.match(actionResult)) {
  ...
  const handleSave = async () => {
    ...
    if (updateProfileDesigner.fulfilled.match(actionResult)) {
      ...
      setIsEditing(false);
    } else if (updateProfileDesigner.rejected.match(actionResult)) {
      toast.error(actionResult.payload.message);
    }
  };
